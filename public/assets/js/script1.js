...
function TestimonialsModuleInitialize_Layout1_Layout8_Layout9() {
  $(document).on("s123.page.ready", function (event) {
    var $sections = $(
      ".s123-module-testimonials.layout-1, .s123-module-testimonials.layout-8, .s123-module-testimonials.layout-9"
    );
    $sections.each(function (index) {
      var $s = $(this);
      var $carousel = $s.find('[data-ride="carousel"]');
      var layout_customize =
        $s.find(".layout-customize").length > 0
          ? tryParseJSON($s.find(".layout-customize").val())
          : "";
  ...
  function setTestimonialsHeight() {
    ...
    $sections.each(function (index) {
      var $s = $(this);
      var $carousel = $s.find('[data-ride="carousel"]');
      $carousel.find(".item, .testimonial-image").css({ minHeight: "" });
      if (!IsIE11()) {
        var selector =
          $s.hasClass("layout-8") && findBootstrapEnvironment() !== "xs"
            ? ".testimonial-image"
            : ".item";
        $carousel.find(selector).css({
          minHeight: Math.max.apply(
            Math,
            $carousel.find(".item").map(function () {
...
jQuery(function ($) {
  TestimonialsModuleInitialize_Layout12();
});
function TestimonialsModuleInitialize_Layout12() {
  $(document).on("s123.page.ready", function (event) {
    var $sections = $(".s123-module-testimonials.layout-12");
    $sections.each(function (index) {
      var $s = $(this);
      var $categories = $s.find(".testimonials-category");
      var categories = new ModuleLayoutCategories({
      ...
      var $carousel = $($s.find(".testimonials-continuous-slider").get(0));
      var slidesToShow = whatScreen.any() == "mobile" ? 1 : 3;
      $(window).on("resize", function () {
        if (whatScreen.any() == "mobile") {
          slidesToShow = 1;
        } else {
          slidesToShow = 3;
        }
        $carousel.slick("destroy");
        testimonialsLayout12Slick($carousel, slidesToShow);
      });
      if (!$carousel.hasClass("slick-initialized")) {
        testimonialsLayout12Slick($carousel, slidesToShow);
      }
      $s.on("module_layout_categories.show", function (event, category) {
        $carousel.slick("destroy");
        $carousel = $s.find(
          '.testimonials-category[data-categories-filter="' +
            category +
...
function testimonialsLayout12Slick($carousel, slidesToShow) {
  $carousel.slick({
    slidesToShow: slidesToShow, // number of slides to show at once
    slidesToScroll: 1, // number of slides to scroll at once
    autoplay: true,
    infinite: true,
    speed: 8000, // speed of the transition, play with this value to achieve desired effect
...
function headersModuleInitialize_Layout() {
  $(document).on("s123.page.ready", function (event) {
    var $sections = $(
      "section.s123-module-headers:is(.layout-6,.layout-7,.layout-8,.layout-9,.layout-18,.layout-20,.layout-22,.layout-23,.layout-24,.layout-25,.layout-26,.layout-27,.layout-29,.layout-30,.layout-35,.layout-39,.layout-40)"
    );
    $sections.each(function (index) {
      var $s = $(this);
      var $carousel = $s.find('[data-ride="carousel"]');
      ...
      if (
        sliderSpeed == "" ||
        parseInt(sliderSpeed) < 1 ||
        parseInt(sliderSpeed) > 21
      ) {
        sliderSpeed = 5000;
      } else {
        sliderSpeed = parseInt(sliderSpeed) * 1000;
      }
      $s.find(".header1, .header2").each(function (index) {
        var $this = $(this);
        if ($this.data("line-height") && $this.data("line-height").length > 0) {
          $this
            .get(0)
            .style.setProperty(
              "--header_line_height",
      ...
      if ($s.hasClass("layout-20")) {
        ...
        if ($firstImage.length > 0) {
          if ($firstImage.data("bg") !== undefined) {
            var img = new Image();
            img.src = $firstImage.data("bg");
            img.onload = function () {
              var aspectRatio = this.width / this.height;
              $s.find(".headers-image").css(
                "aspect-ratio",
                String(aspectRatio)
              );
              if (
                $s.find(".headers-container").hasClass("circle-under-image") &&
                aspectRatio < 1
              ) {
            ...
            $firstImage
              .closest(".headers-img-wrap")
              .addClass("circle-fixed-width");
          }
        }
      }
      if ($s.hasClass("layout-22") || $s.hasClass("layout-29")) {
        ...
        if ($headersimage.length == 0) {
          $headersimage = $s.find(".headers-bg-video");
        }
        if ($headersimage.length > 0 && $headersDescription.length > 0) {
          if (
            $headersDescription.get(0).offsetHeight - 60 >
            $headersimage.get(0).offsetHeight
          ) {
            $s.get(0).style.setProperty(
              "--headers-description-height",
              $headersDescription.get(0).offsetHeight + "px"
            );
          }
        }
      }
      $carousel.carousel({ interval: sliderSpeed });
      if (
        $s.hasClass("layout-26") &&
        $s.attr("data-version") !== undefined &&
        $s.attr("data-version") == "2"
      ) {
        if (IsWizard()) {
          $(document).on("site123.colorPicker.change", function (event) {
      ...
      function updateBackgroundColor() {
        var boxColorClass = $s.data("box-color");
        var color = $s.hasClass("bg-primary")
        ...
        switch (boxColorClass) {
          case "bg-primary-gray":
            color = "#ebedf0";
            break;
          case "bg-primary-white":
            color = "#ffffff";
            break;
          case "bg-primary-black":
            color = "#000000";
            break;
          case "background-primary-color btn-primary-text-color":
            color = $s.hasClass("bg-primary")
              ? getComputedStyle(document.documentElement).getPropertyValue(
                  "--modules_color_section_main"
                )
              : getComputedStyle(document.documentElement).getPropertyValue(
                  "--modules_color_second_section_main"
                );
            break;
        }
        $s.find(".headers-img-wrap")
          .get(0)
          .style.setProperty("--layout26OpacityColor", hexToRGB(color));
      }
      ...
      if ($s.data("image-width") == 2) {
        var sectionHeight = $s.height();
        $s.find(".headers-container").css("min-height", sectionHeight + "px");
        if ($s.hasClass("layout-22")) {
          $s.find(".headers-image").css("min-height", sectionHeight + "px");
        }
        $s.data("image-width", 0);
      }
    });
    var $sections = $("section.s123-module-headers");
    $sections.each(function (index) {
      var $s = $(this);
      $s.find(".custom-header-font").each(function (index) {
        var $this = $(this);
        var customFont = $this.data("custom-font");
        if (customFont.length > 0) {
          $this.css("font-family", customFont);
        }
      });
      $s.find(".header1, .header2").each(function (index) {
        var $this = $(this);
        if (
          $s.hasClass("layout-7") ||
          $s.hasClass("layout-9") ||
          $s.hasClass("layout-22") ||
          $s.hasClass("layout-23") ||
          $s.hasClass("layout-24") ||
          $s.hasClass("layout-26") ||
          $s.hasClass("layout-27") ||
          $s.hasClass("layout-29") ||
          $s.hasClass("layout-33")
        ) {
          fitHeaderTextToParentContainer($this);
        }
      });
      function fitHeaderTextToParentContainer($textEl) {
        if ($textEl.length == 0) return;
        fontSize = parseFloat($textEl.css("font-size"));
        if (!$.isNumeric(fontSize)) return;
        var $parentContainer = $textEl.closest(
          ".headers-text-resize-container"
        );
        if (
          $parentContainer.length > 0 &&
          $parentContainer.width() < $textEl.width()
        ) {
          $textEl.css("visibility", "hidden");
          var parentContainerWidth = $parentContainer.width();
          if (
            $textEl.closest("section").hasClass("layout-9") ||
            $textEl.closest("section").hasClass("layout-27")
          ) {
            parentContainerWidth = parentContainerWidth + 5;
          }
          if ($textEl.closest("section").hasClass("layout-23")) {
            parentContainerWidth = $parentContainer.innerWidth();
          }
          var index = 0;
          while (parentContainerWidth < $textEl.width() && index < 99) {
            index++;
            fontSize--;
            $textEl.get(0).style.setProperty("--header-font-size", fontSize);
          }
          $textEl.css("visibility", "unset");
        }
      }
    });
    S123_ActionButtons.init();
  });
}
jQuery(function ($) {
    ...
    $section.each(function (index) {
      ...
      $sectionThis.find(".contactUsForm").each(function (index) {
        ...
        $form.validate({
          ...
          focusInvalid: true,
          ignore:
            ':hidden:not(.custom-form-step:visible input[name^="datePicker-"])',
          ...
          submitHandler: function (form) {
            ...
            var url =
              "/versions/" + $("#versionNUM").val() + "/include/contactO.php";
            if (
              $form.hasClass("custom-form") ||
              $form.hasClass("horizontal-custom-form")
            ) {
              ...
              url =
                "/versions/" +
                $("#versionNUM").val() +
                "/include/customFormO.php";
            }
            if (
              forms_GoogleRecaptcha.isActive &&
              !forms_GoogleRecaptcha.isGotToken
            ) {
                ...
                if (clickAction == "thankYouMessage" || clickAction == "") {
                  ...
                  if (top.$("#websitePreviewIframe").length) {
                    bootbox.alert({
                      ...
                      message: translations.previewExternalLinkMsg.replace(
                        "{{externalLink}}",
                        "<b>" + dataObj.action.url + "</b>"
                      ),
        ...
        $form.find(".f-b-date-timePicker").each(function () {
          ...
          var $hiddenInput = $option.find(
            '[data-id="' + $datePicker.data("related-id") + '"]'
          );
