...
// esta funcion permite cerrar un cuadro modal que se usa para mostrar los comentarios de la cotizacion
function cierraModalComentario() {
    $('#miModalAdicionalComentario').modal('hide');
}  

// esta funcion guarda el comentario de la cotizacion en un cuadro de texto invisible 
...
// cVarComentario es el comentario que viene de una cotizacion desde la base de datos 
...
function searchCotizacion(){
    ...
    while (!esNumeroEntero) {
        inputUsuario = prompt("Numero de Cotización a Buscar");
        ...
        if (!isNaN(numero) && Number.isInteger(numero)) {
            ...
            searchCotizacion2(inputUsuario);
        } else {
...
function searchPrev(){
    ...
    if (numero > 0){
        ...
        alert ("El numero de cotización es inválido")
    }
}
...
/*
// esta funcion permite cargar los detalles de la cotizacion el encabezado la página
@ params datos Arreglo que contiene los datos de  la cotización
*/
function viewDetallesCotizacion1(cotizacion){
    try {
        console.log ("entrando viewDetallesCotizacion1 ");
        /*
            anulada: "0"
            aux01: ".00000"
            aux02: "                              "
        ...
        */
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error viewDetallesCotizacion1");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }  
...
// esta funcion permite cargar los datos de una cotizacion del 2k12
function viewDetallesCotizacion12K12(cotizacion){
    try {
        console.log ("entrando viewDetallesCotizacion1 2k12 ");
        /*
        {
        "doc_num": "000002              ",
        "descrip": null,
        ...
        */
        // verificamos si esta procesada
        console.log ("arreglo cotizacion 2k12",cotizacion);
        ...
        if ((cVarAnulada=="1") ||(cVarAnulada==1)){
            // cambiamos la image de anular cotizacion por activar cotizacion
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error viewDetallesCotizacion1 2k12");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }  
...
/*
// esta funcion permite cargar los detalles de los renglones de la cotizacion 
// en el grid de la página
@ params datos Arreglo que contiene los datos de  la cotización
*/
function viewDetallesCotizacion(datos){
    try {
        console.log ("entrando viewDetallesCotizacion 2k8 Renglones");  
        paso=1;  
        ...
        for (i=0; i< n; i++){
            ...
            */
            ...
            cadenaHTML=cadenaHTML+`<td class='text-center td-cotizacion'><input type='text' id='almacen${i}' name='almacen' value='${cAlmacen}' size='3' onchange='javascript:changeAlmacen("${nRenNum}',"almacen${i}")'></td>`;
            cadenaHTML=cadenaHTML+`<td class='text-right td-cotizacion'><input type='text' id='cantidad${i}' name='cantidad' value='${parseInt(fCant)}' size='8' style='text-align:right !important' onChange='javascript:actualizaReng("${nRenNum}")'></td>`;
            cadenaHTML=cadenaHTML+`<td class='td-cotizacion'>${nUnidad}</td>`;
            cadenaHTML=cadenaHTML+`<td class='text-right td-cotizacion'><input type='text' id='precio${i}' name='precio' value='${fPrecio1}' size='8' style='text-align:right !important'onChange='javascript:actualizaReng("${nRenNum}")'></td>`;
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error viewDetallesCotizacion 2k8 Renglones");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }          
...
/*
// esta funcion permite cargar los detalles de los renglones de la cotizacion del 2k12
// en el grid de la página
@ params datos Arreglo que contiene los datos de  la cotización
*/
function viewDetallesCotizacion2k12(datos){
    try {
        console.log ("entrando viewDetallesCotizacion 2k12 Renglones");  
        paso=1;  
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error viewDetallesCotizacion 2k12 Renglones");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }          
...
// esta funcion permite actualizar el almacen del renglon activo
function changeAlmacen(nReng,objName){
    try {
        console.log ("Entrando en changeAlamcen");
        paso=1;
        console.log (`renglon: ${nReng}`);
        paso=3;
        console.log (`objeto: ${objName}`);
        paso=4;
        console.log (myPersistentArray);
        cAlmacen=document.getElementById(objName).value.trim().toUpperCase();
        document.getElementById(objName).value=cAlmacen;
        myPersistentArray[(nReng-1)].cAlmacen=cAlmacen;
        paso=5;
        // redibujamos la tabla
        drawTableDatail(myPersistentArray);
    } catch (error) {
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error changeAlamcen");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }     
}
...
/*
// esta funcion permite cargar los detalles de la cotizacion en el grid de la página
@ params datos Arreglo que contiene los datos de  la cotización
*/
...
// esta funcion es la que ejecuta la busqueda en si de la cotización
// utiliza el parametro numero el cual es entero y se valida en searchCotizacion
function searchCotizacion2(numero){
    // limpiamos los comentarios
    limpiarComentarios();
    //------------------------
    $.ajax({
        url: "/search_cotizacion",
        type: "get",
        dataType: "html",
        data: {
            idCotizacion:numero,
        ...
        if (data.valor==1){
            ...
            cotizacionesAnteriores=data.cotizacionesAnteriores;
            // determinamos el origen de la cotización
            origenCotizacion=data.origen;
            console.log ("origen de la cotización:",origenCotizacion);
            console.log ("Datos de la Cotizacion:",cotizacion);
            console.log ("Datos de los renglones de cotizacion:",datos);
            console.log ("Datos del cliente:",cliente);
            // limiamos los elementos del arreglo persistente
            cleanAll();
            // cargamos los detalles de la cotizacion
            if (origenCotizacion=="2K8"){
                // cargamos los detalles de la cotizacion
                viewDetallesCotizacion1(cotizacion);
                ...
                // cargamos los detalles de la cotizacion 
                viewDetallesCotizacion12K12(cotizacion);
                // cargamos los detalles de los renglones de cotizacion
                // esto tambien carga el arreglo persistente de los renglones
                // de pedido                
                viewDetallesCotizacion2k12(datos);
            ...
            if (data.valor==-1){
                alert ("Esta cotización no existe, por favor verifique los datos");
            } else {
                alert ("Ocurrió un error que no pudo ser controlado\nInténtelo de nuevo");
            }
...
function searchCotizacion3(numero){
    try {
        ...
        paso=4; 
        // ejecutamos la busqueda de la cotizacion
        searchCotizacion2(numero);
    } catch (error) {   
        // Código que se ejecuta si hay un error en el bloque try
...
// esta funcion permite navegar a la cotización seleccionada en la lista
function navegarACotizacion(numeroCotizacion) {
    console.log("Navegando a la cotización:", numeroCotizacion);
    ...
    if (elementoCotizacion) {
        ...
        console.warn(`No se encontró la cotización con el ID: ${idAncla}`);
    }
}
...
// buscamos el comentario de cotizacion usando el codigo del articulo y el renglon de cotizacion
...
function newCotizacion(){
    ...
    var r=confirm("Desea abandonar el proceso y crear una nueva cotización\nLa información que no haya guardado se perderá");
    if (r){
        document.getElementById('numero').value="";
        document.location="/nueva_cotizacion";
    }
}
        ...
        if (r){
            ...
            $.ajax({
                ...
                if (data["valor"]=="1"){
                    ...
                    if (mensaje.includes("SQLSTATE[23000]")){
                        paso=18;
                        msgError (`No puede eliminar esta cotizacion porque el número no existe<br>Número de cotizacion ${cotizacion}`);
                    } else {
                        paso=19;
                        msgError (`${mensaje}`);
                    }
                    ...
                    paso=15;
                    msgError ("Error al intentar eliminar la cotización, intenelo mas tarde");
                } else {
                    // error inesperado
                    paso=15;
            ...
            .fail(function (jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status === 500) {
                    ...
                    if (mensaje.includes("SQLSTATE[23000]")){
                        paso=59;
                        msgError (`No puede eliminar esta cotizacion porque el numero no existe<br>Número de cotizacion ${cotizacion}`);
                    } else {
                        paso=60;
                        msgError (`${mensaje}`);
                    }
    ...
    try {
        ...
        */
        ...
        if (
            ...
            if (!encontrado){
                msgError("No puede crear una cotización vacía, por favor agregue elementos a la misma");
                return (false);
            }
            ...
            if (numeroCotizacionV.trim()!=""){
                ...
                $.ajax({
                    ...
                    if (data["valor"]=="1"){
                        ...
                        if (mensaje.includes("SQLSTATE[23000]")){
                            paso=18;
                            msgError (`No puede actualizar esta cotizacion porque el numero no existe<br>Número de cotizacion ${cotizacion}`);
                        } else {
                            paso=19;
                            msgError (`${mensaje}`);
                        }
                    } else if (data["valor"]=="-3") {
                        // error al actuzlizar la cotizacion
                        paso=15;
                        msgError ("Error al intentar actualizar la cotización, intenelo mas tarde");
                    } else {
                        // error inesperado
                        paso=15;
                        msgError ("Ocurrió un error que no pudo ser controlado<br>Inténtelo de nuevo");
                ...
                .fail(function (jqXHR, textStatus, errorThrown) {
                    ...
                    if (jqXHR.status === 500) {
                        ...
                        if (mensaje.includes("SQLSTATE[23000]")){
                            paso=59;
                            msgError (`No puede crear esta cotizacion porque el numero ya existe<br>Número de cotizacion ${cotizacion}`);
                        } else {
                            paso=60;
                            msgError (`${mensaje}`);
                        }
                ...
                // creando una cotizacion nueva
                $.ajax({
                    url: "/save_new_cotizacion",
                    type: "POST",
                    dataType: "html",
                    data:{
                        numeroCotizacion:numeroCotizacionV,
                    ...
                    //console.log  (data.valor);                                   
                    if (data["valor"]=="1"){
                        // se creo la cotizacion con éxito
                        paso=13;
                        estado=data["estado"];
                        resultado=data["resultados"];
                        $numeroCotizacion=data["cotizacion"];
                        ...
                        // error al crear la cotizacion, se debe a que uno de los renglones no tiene uniad
                        // asignada
                        paso=14;
                        estado=data["status"];
                        paso=15;
                        ...
                        if (mensaje.includes("SQLSTATE[23000]")){
                            paso=18;
                            msgError (`Ocurrió un error al crear los renglones de cotizacion porque uno de los artículos no tiene unidad asignada<br>Número de cotizacion ${cotizacion}`);
                        } else {
                            paso=19;
                            msgError (`${mensaje}`);
                        }
                        ...
                        // error de SQL
                        paso=15;
                        msgError ("Error al intentar inserta la cotización, intenelo mas tarde");
                    } else {
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error saveCotizacion!");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }     
...
function printCotizacion(){
    try {
        console.log ("entrando a printCotizacion");
        paso=1;
        ...
        if (!Number.isNaN(numeroCotizacionV) &&(typeof(numeroCotizacionV)!="undefined") && (parseInt(numeroCotizacionV)>0)){
            paso=4;
            //window.open('/imprimir_cotizacion/?numeroCotizacion='+numeroCotizacion+"&db="+db+"&arregloDatos="+arregloDatos, '_blank',"width=1240,height=800"); 
                $.ajax({
                    url: "/imprimir_cotizacion",
                    type: "POST",
                    dataType: "html",
                    data:{
                        numeroCotizacion:numeroCotizacionV,
            ...
            paso=7;
            cadena="Debe suministrar un numero de cotización para imprimir";
            paso=8;
            msgError (cadena);
        }
    } catch (error) {
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error printCotizacion");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }     
    ...
    try {
        ...
        for (i=0; i < n; i++){
            ...
            */
            ...
            cadenaHTML=cadenaHTML+`<td class='text-center td-cotizacion'><input type='text' id='almacen${i}' name='almacen' value='${cAlmacen}' size='3' onchange='javascript:changeAlmacen("${nRenNum}","almacen${i}")'></td>`;
            paso=38;
            cadenaHTML=cadenaHTML+`<td class='text-right td-cotizacion'><input type='text' id='cantidad${i}' name='cantidad' value='${parseInt(nCant)}' size='8' style='text-align:right !important' onChange='javascript:actualizaReng("${nRenNum}")'></td>`;
            paso=39;
            cadenaHTML=cadenaHTML+`<td class='td-cotizacion'>${nUnidad}</td>`;
            paso=40;
            cadenaHTML=cadenaHTML+`<td class='text-right td-cotizacion'><input type='text' id='precio${i}' name='precio' value='${fPrecio1}' size='8' style='text-align:right !important' onChange='javascript:actualizaReng("${nRenNum}")'></td>`;
        ...
        // actualizamos el subtotal de la cotizacion
        paso=53;
        ...
        // actualizamos el total de la cotizacion
        paso=56;
...
// esta funcion permite agrega un nuevo item a la cotización
function addItem(){
    try {
        ...
        //---- totalizamos la cotizacion ---------------//
        paso=11;
        recalCotizacion(myPersistentArray);
...
// esta funcion permite eliminar un item de la cotizacion
function deleteItem(){
    try {
        ...
        // ejecutamos el recalculo de la cotizacion;
        paso=15;
        recalCotizacion(myPersistentArray);
    } catch (error) {
        // Código que se ejecuta si hay un error en el bloque try
...
function recalCotizacion(myPersistentArray){
    try {
        console.log ("entrando a recalCotizacion");
        paso=1;
        var n=myPersistentArray.length;
        ...
        // Código que se ejecuta si hay un error en el bloque try
        console.error("¡Ha ocurrido un error recalCotizacion");
        console.error("Mensaje de error:", error.message,"paso:",paso);
        console.error("Tipo de error:", error.name);
    }     
...
function processEsp1(){
    try {
        ...
        paso=12;
        window.open ("/special_process_one/?empresa="+empresaV+"&dolarHP="+dolarHPV,"_special_process_one",`scrollbars=yes;resizable=yes,width=${anchoVentana},height=${altoVentana},left=${izquierda},top=${arriba}`);
   } catch (error) {
        // Código que se ejecuta si hay un error en el bloque try
...
// esta funcion muestra los comentarios de la cotizacion en la division adicionales
...
// esta funcion permite anular una cotizacion en el sistema nuevo
